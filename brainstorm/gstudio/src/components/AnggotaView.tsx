...
import { Home, PiggyBank, Users, CreditCard, User, CheckCircle2, ShieldAlert, Award, FileText, Upload, AlertCircle, Sparkles, Send, Eye, ChevronRight } from 'lucide-react';
...
export default function AnggotaView({
  ...
  const [activeTab, setActiveTab] = useState<'beranda' | 'simpan' | 'grup' | 'pinjaman' | 'profil'>('beranda');

  // Active Member context
  const currentMember = members.find(m => m.id === activeMemberId) || members[0];
  const currentGroup = groups.find(g => g.anggotaIds.includes(currentMember.id));
  const currentLoan = loans.find(l => l.memberId === currentMember.id && l.status !== 'Lunas');

  // Local states
  const [showQrisModal, setShowQrisModal] = useState(false);
  const [qrisConfig, setQrisConfig] = useState<{ title: string; amount: number; onConfirm: () => void } | null>(null);
  ...
  const handleSendOtp = () => {
    ...
    setOtpSent(true);
    alert('Simulasi SMS OTP dikirim ke ' + kycForm.noHp + '. Gunakan kode OTP palsu: "1234"');
  };
  ...
  const handleSubmitKyc = (e: React.FormEvent) => {
    ...
    addLog(kycForm.nama, 'Daftar & e-KYC', `Mengajukan pendaftaran anggota baru NIK ${kycForm.nik}.`);
    alert('Pendaftaran Berhasil dikirim! Status akun Anda sekarang: Requested. Pengurus harus menyetujui di Panel Pengurus agar akun Anda aktif.');
    setKycStep(1); // reset step
    setActiveTab('beranda');
  };

  // Pay Savings or Loan Installment via QRIS
  const triggerQris = (title: string, amount: number, onConfirm: () => void) => {
    ...
    triggerQris(`Simpanan ${saveType}`, amount, () => {
      ...
      setMembers(prev => prev.map(m => {
        if (m.id === currentMember.id) {
          ...
          updated.skorKeanggotaan = Math.min(100, updated.skorKeanggotaan + 2); // bonus score for savings
          return updated;
        }
        return m;
      }));
      ...
      addLog(currentMember.nama, 'Tabungan QRIS', `Berhasil menabung Simpanan ${saveType} senilai ${formatIdr(amount)}.`);
      alert(`Alhamdulillah, Simpanan ${saveType} sebesar ${formatIdr(amount)} telah sukses disetor dan dicatat otomatis ke Immutable Ledger.`);
    });
  };
  ...
  const handleApplyLoan = (e: React.FormEvent) => {
    ...
    if (!currentGroup) {
      alert('Anda belum memiliki kelompok tanggung renteng! Silakan gabung atau buat grup terlebih dahulu di menu "Grup".');
      return;
    }
    ...
    if (nominal > currentGroup.plafonMaks) {
      score -= 25;
      flagReasons.push(`Pengajuan (${formatIdr(nominal)}) melebihi batas plafon kelompok (${formatIdr(currentGroup.plafonMaks)}).`);
    }

    // Custom check for simulated case 'Pak X'
    if (currentMember.nama.includes('Pak X') || currentMember.id === 'm3') {
      score = 48;
      flagReasons.push('Terdeteksi tunggakan tersembunyi di Koperasi Tetangga (terverifikasi lintas data).');
    }
    ...
    const newLoan: Loan = {
      ...
      skorAi: score,
      flagAi: flagAi,
      flagAlasan: flagReasons.length > 0 ? flagReasons : ['Kapasitas mencukupi', 'Grup aktif saling menjamin'],
      isSanggah: false
    };

    setLoans(prev => [...prev, newLoan]);
    addLog(currentMember.nama, 'Pengajuan Pinjaman', `Mengajukan pinjaman ${formatIdr(nominal)} untuk ${loanPurpose}. AI Skor: ${score} (${flagAi}).`);
    alert(`Pengajuan pinjaman sebesar ${formatIdr(nominal)} berhasil dikirim dan terikat ke Kelompok "${currentGroup.nama}". Hasil skrining awal AI: ${flagAi} (Skor ${score}).`);
  ...
  const handleSanggahSubmit = (loanId: string) => {
    ...
    addLog(currentMember.nama, 'Hak Sanggah', `Mengajukan sanggahan atas flag pinjaman: "${sanggahText}"`);
    alert('Sanggahan Anda berhasil direkam ke sistem! Pengurus akan meninjau alasan ini pada musyawarah verifikasi kelompok.');
    setSanggahText('');
    setShowSanggahArea(false);
  };
  ...
  const handlePayInstallment = () => {
    ...
    triggerQris(`Cicilan Bulanan`, currentLoan.cicilanBulanan, () => {
      ...
      addLog(currentMember.nama, 'Bayar Cicilan', `Membayar cicilan bulanan ${formatIdr(currentLoan.cicilanBulanan)} via QRIS.`);
      alert(`Alhamdulillah, pembayaran cicilan sebesar ${formatIdr(currentLoan.cicilanBulanan)} sukses! Skor keanggotaan Anda naik.`);
    });
  };

  // Create Group or Join
  const [newGroupForm, setNewGroupForm] = useState({ nama: '', plafon: '5000000', jadwal: 'Setiap tanggal 5' });
  const handleCreateGroup = (e: React.FormEvent) => {
    ...
    const generatedCode = (newGroupForm.nama.trim().replace(/[^a-zA-Z0-9]/g, '').substring(0, 5).toUpperCase() + Math.floor(10 + Math.random() * 90)).padEnd(6, 'X');
    ...
    addLog(currentMember.nama, 'Bentuk Grup', `Membentuk kelompok tanggung renteng baru: "${newGroupForm.nama}" dengan kode undangan "${generatedCode}".`);
    alert(`Kelompok "${newGroupForm.nama}" berhasil dibuat dengan Kode Undangan: ${generatedCode}! Anggota lain sekarang dapat bergabung dengan grup Anda menggunakan kode tersebut.`);
    setNewGroupForm({ nama: '', plafon: '5000000', jadwal: 'Setiap tanggal 5' });
  };
  ...
  const handleJoinGroup = (groupId: string) => {
    ...
    addLog(currentMember.nama, 'Gabung Grup', `Bergabung dengan kelompok tanggung renteng "${joinedG?.nama || groupId}".`);
  ...
  const handleJoinByInviteCode = (e: React.FormEvent) => {
    ...
    const targetGroup = groups.find(g => g.kodeUndangan && g.kodeUndangan.toUpperCase() === code);
    if (!targetGroup) {
    ...
    addLog(currentMember.nama, 'Gabung via Undangan', `Bergabung ke kelompok "${targetGroup.nama}" menggunakan kode undangan "${code}".`);
    alert(`Selamat! Anda berhasil bergabung dengan Kelompok Tanggung Renteng "${targetGroup.nama}".`);
    setInviteCodeInput('');
  };
  ...
  return (
    <div className="w-full max-w-[390px] mx-auto bg-[#1E1F21] rounded-[55px] p-3 shadow-[0_24px_50px_rgba(30,31,33,0.12)] border-[10px] border-[#1E1F21] relative flex flex-col justify-between overflow-hidden h-[844px] text-[#1E1F21] font-sans">
      ...
      <div className="bg-gradient-to-br from-[#F06A6A] via-[#E5544F] to-[#C83E3E] text-white pt-5 pb-6 px-5 shrink-0 z-20 shadow-sm relative overflow-hidden select-none">
        ...
        <div className="flex justify-between items-center mb-4 relative z-10">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-black text-[9px] tracking-wider text-white uppercase">Koperasi Desa Merah Putih</span>
          </div>
          ...
          <div className="flex items-center gap-1 bg-white/12 hover:bg-white/18 px-2.5 py-1.5 rounded-full border border-white/12 cursor-pointer transition-all relative">
            <span className="text-[10px] font-extrabold text-white">{currentMember.nama}</span>
            <select 
              value={activeMemberId}
              onChange={(e) => {
                setActiveMemberId(e.target.value);
                addLog('Sistem', 'Pindah User HP', `Pindah ke akun HP anggota: ${members.find(m => m.id === e.target.value)?.nama}`);
              }}
              className="absolute inset-0 opacity-0 cursor-pointer w-full h-full"
            >
              {members.map(m => (
                <option key={m.id} value={m.id} className="text-slate-800 font-semibold">{m.nama}</option>
              ))}
            </select>
            <svg className="w-3 h-3 text-slate-200 fill-current" viewBox="0 0 20 20">
        ...
        <div className="relative z-10 mt-1">
          <span className="text-[9px] text-red-100 font-bold uppercase tracking-widest block">Ringkasan Keuangan</span>
          <h2 className="text-2xl font-black text-white mt-1 select-all">
            {formatIdr(currentMember.simpananPokok + currentMember.simpananWajib + currentMember.simpananSukarela)}
          </h2>
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-[10px] text-red-50 font-medium">Aset Kelompok: <strong className="text-white">Rp15.250.000</strong></span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#FAF9F8] border-b border-[#E4E4E4] py-2 px-3 text-[9px] text-[#6D6E6F] font-medium flex items-center gap-1.5 shrink-0 z-20">
        ...
        <span className="leading-tight">Keputusan akhir tetap mufakat musyawarah grup & pengurus. AI adalah asisten EWS.</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-[#FAF9F8] p-4">
        ...
        {currentMember.statusKyc === 'Requested' && (
          <div className="bg-amber-100 border border-amber-300 rounded-xl p-3 text-xs text-amber-900 mb-4 flex items-start gap-2">
            ...
            <div>
              <strong>Akun e-KYC Sedang Ditinjau!</strong> Pendaftaran Anda telah dikirim. Harap tunggu persetujuan dari Pengurus Koperasi di panel pengurus untuk mengaktifkan seluruh fitur pembiayaan.
            </div>
          </div>
        )}
        ...
        {currentMember.statusKyc === 'Rejected' && (
          <div className="bg-red-100 border border-red-300 rounded-xl p-3 text-xs text-red-900 mb-4 flex items-start gap-2">
            ...
            <div>
              <strong>KYC Ditolak!</strong> Pengajuan verifikasi KTP Anda ditolak oleh pengurus. Silakan hubungi kantor koperasi terdekat atau perbarui e-KYC.
            </div>
          </div>
        )}
          ...
          >
            ...
            <div className="flex justify-between items-center bg-white p-3 rounded-2xl border border-slate-200/60 shadow-sm">
              <div className="flex items-center gap-2.5">
                <div className="w-10 h-10 rounded-full bg-[#FCE8E6] border border-[#F06A6A]/10 flex items-center justify-center text-[#F06A6A] font-black text-sm shadow-inner">
                  {currentMember.nama.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                </div>
                <div>
                  <span className="text-[10px] text-[#F06A6A] font-extrabold block uppercase tracking-wider">Anggota Koperasi</span>
                  <h3 className="text-xs font-black text-[#1E1F21] leading-tight">{currentMember.nama}</h3>
                  <p className="text-[9px] text-[#6D6E6F] font-semibold">{currentMember.pekerjaan} · {currentMember.alamat}</p>
                </div>
              </div>
            ...
            <div>
              <div className="flex justify-around items-center bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm">
                ...
                >
                  ...
                  <span className="text-[10px] font-extrabold text-[#1E1F21] tracking-tight">Setor Simpanan</span>
                </button>
                ...
                >
                  ...
                  <span className="text-[10px] font-extrabold text-[#1E1F21] tracking-tight">Bayar Cicilan</span>
                </button>
                ...
                >
                  ...
                  <span className="text-[10px] font-extrabold text-[#1E1F21] tracking-tight">Kelompok Kami</span>
                </button>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-white to-[#F6F5F3] p-4 rounded-2xl border border-slate-200/60 shadow-sm relative overflow-hidden">
              <div className="mr-28 space-y-1.5 relative z-10">
                ...
                <p className="text-[10px] text-[#6D6E6F] leading-relaxed font-semibold">
                  RantaiRenteng mendigitalkan gotong-royong. Tanpa jaminan fisik, digantikan dengan mufakat tanggung renteng kelompok.
                </p>
                <button 
                  onClick={() => setActiveTab('grup')}
              ...
              <div className="absolute right-3.5 bottom-3.5 w-20 h-20 pointer-events-none flex items-center justify-center">
                <div className="relative w-14 h-14 transform rotate-12">
                  <div className="absolute inset-0 bg-gradient-to-tr from-[#F06A6A] to-[#E5544F] rounded-lg shadow-md transform -skew-x-12 skew-y-6 flex items-center justify-center border border-white/20">
                    <span className="text-white/90 font-mono text-[9px] font-black">RP</span>
                  </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[9px] text-slate-400 font-bold font-mono uppercase">Skor</span>
              </div>
              ...
              <div className="space-y-2.5">
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#5C0A1A] rounded-full" style={{ width: '100%' }}></div>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#5C0A1A] rounded-full" style={{ width: '82%' }}></div>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#5C0A1A] rounded-full" style={{ width: '92%' }}></div>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#5C0A1A] rounded-full" style={{ width: '76%' }}></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <h4 className="text-[10px] font-black text-slate-400 mb-2 uppercase tracking-wider">Pinjaman Berjalan</h4>
              {currentLoan ? (
                <div className="bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h5 className="text-xs font-black text-slate-800 mt-1">{currentLoan.tujuan}</h5>
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-black text-[#5C0A1A] block">{formatIdr(currentLoan.nominal)}</span>
                      <span className="text-[8px] text-slate-400 block font-bold">{currentLoan.tenor} Bulan Tenor</span>
                    </div>
                  </div>

                  <div className="border-t border-slate-100 pt-2 flex justify-between items-center text-[11px] font-medium text-slate-600">
                    <div>
                      <span className="text-[9px] text-slate-400 block font-bold">Angsuran / Bulan</span>
                      <strong className="text-slate-800 font-bold">{formatIdr(currentLoan.cicilanBulanan)}</strong>
                    </div>
                    <div className="text-right">
                      <span className="text-[9px] text-slate-400 block font-bold">Sisa Tenor</span>
                      <strong className="text-slate-800 font-bold">{currentLoan.sisaCicilan} bln lagi</strong>
                    </div>
                  </div>
                  ...
                  {currentLoan.statusCicilan === 'UNPAID' && (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-2 flex justify-between items-center mt-2">
                      <div className="flex items-center gap-1.5 text-amber-800 text-[10px]">
                        ...
                        <span className="font-semibold">Jatuh tempo {currentLoan.jadwalCicilan}</span>
                      </div>
                      <button
                  ...
                  {currentLoan.statusCicilan === 'DITALANGI' && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-2.5 text-[10px] text-red-900 space-y-1.5">
                      <p className="font-black flex items-center gap-1 text-red-700 uppercase tracking-wide">
                        <AlertCircle className="w-3.5 h-3.5" /> Tanggung Renteng Aktif (Grup Menalangi)
                      </p>
                      <p className="text-slate-600 leading-relaxed">Cicilan Anda ditalangi kas kelompok. Harap segera bayar ganti talangan kelompok untuk mengembalikan skor Anda.</p>
                      <button
                        onClick={handlePayInstallment}
                ...
                <div className="bg-white p-4 rounded-xl border border-dashed border-slate-300 text-center py-5 text-slate-400 text-xs">
                  ...
                  <button onClick={() => setActiveTab('pinjaman')} className="block mx-auto mt-2 text-[#5C0A1A] font-black bg-[#5C0A1A]/5 hover:bg-[#5C0A1A]/10 px-3 py-1 rounded-full border border-[#5C0A1A]/10 text-[10px] uppercase tracking-wider">
            ...
            <div className="bg-white p-3.5 rounded-xl border border-slate-200/60 shadow-sm text-xs space-y-2">
              <span className="text-[9px] text-slate-400 font-bold block uppercase tracking-wider">Afiliasi Tanggung Renteng</span>
              {currentGroup ? (
                <div className="flex justify-between items-center">
                  <div>
                    <h5 className="font-bold text-slate-800">{currentGroup.nama}</h5>
                    <p className="text-[10px] text-slate-400 mt-0.5">Pertemuan: {currentGroup.jadwalPertemuan}</p>
                  </div>
                  <button onClick={() => setActiveTab('grup')} className="text-[10px] font-extrabold text-[#5C0A1A] bg-[#5C0A1A]/5 hover:bg-[#5C0A1A]/10 border border-[#5C0A1A]/10 px-2.5 py-1.5 rounded transition-all">
                    Lihat Grup
                  </button>
                </div>
              ) : (
                <div className="flex justify-between items-center text-slate-500">
                  <span className="font-medium text-slate-500 text-[11px]">Anda belum bergabung kelompok!</span>
                  <button onClick={() => setActiveTab('grup')} className="text-[10px] font-black text-white bg-[#5C0A1A] hover:bg-[#400511] px-3 py-1.5 rounded uppercase tracking-wider transition-all">Join Grup</button>
                </div>
              )}
            </div>
        ...
        {activeTab === 'simpan' && (
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-base font-bold text-slate-800 mt-2">Sektor Tabungan Koperasi</h3>
              <p className="text-xs text-slate-500">Simpanan Anda aman terproteksi di ledger transparan kelompok</p>
            </div>

            {/* Savings Balance summary */}
            <div className="bg-slate-100 rounded-xl p-3 border border-slate-200 grid grid-cols-2 gap-2 text-xs">
              <div>
                <span className="text-slate-400 font-bold text-[9px] block">TOTAL SIMPANAN</span>
                <strong className="text-[#1F3864] text-sm">{formatIdr(currentMember.simpananPokok + currentMember.simpananWajib + currentMember.simpananSukarela)}</strong>
              </div>
              <div className="text-right">
                <span className="text-slate-400 font-bold text-[9px] block">SKOR BONUS SIMPANAN</span>
                <span className="text-[#548235] font-extrabold">+2 Poin Keaktifan</span>
              </div>
            </div>

            <form onSubmit={handleDepositSavings} className="bg-white p-4 rounded-xl border border-slate-200 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Jenis Simpanan</label>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => { setSaveType('Wajib'); setSaveAmount('50000'); }}
                  ...
                  <button
                    type="button"
                    onClick={() => { setSaveType('Sukarela'); setSaveAmount('100000'); }}
                    className={`p-2.5 rounded-lg text-xs font-bold border transition-all ${
                      saveType === 'Sukarela'
                        ? 'border-[#1F3864] bg-[#1F3864]/5 text-[#1F3864]'
                        : 'border-slate-200 bg-white text-slate-600'
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Nominal Setoran (Rp)</label>
                <input
                  type="number"
                  value={saveAmount}
                ...
                />
                {saveType === 'Wajib' && (
                  <span className="text-[10px] text-slate-400 mt-1 block">Simpanan Wajib dipatok Rp50.000 sesuai AD/ART koperasi.</span>
                )}
              </div>
            ...
            <div className="bg-teal-50 border border-teal-200 rounded-xl p-3.5 text-xs text-teal-900 space-y-1">
              ...
              <p className="text-slate-600 text-[11px] leading-relaxed">
                Menabung simpanan wajib secara disiplin sebelum tanggal jatuh tempo meningkatkan <strong>Skor Kelayakan AI</strong> Anda dan memperluas kapasitas pinjaman kelompok.
              </p>
            </div>
          </motion.div>
        )}
        ...
        {activeTab === 'grup' && (
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-base font-bold text-slate-800 mt-2">Kelompok Tanggung Renteng</h3>
              <p className="text-xs text-slate-500">Agunan Sosial & Saling-Menjamin Anggota</p>
            </div>
            ...
            {currentGroup ? (
              <div className="space-y-4">
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Identitas Grup</span>
                  ...
                  <h4 className="text-sm font-bold text-slate-800">{currentGroup.nama}</h4>
                  
                  {/* Invitation Code Section */}
                  <div className="bg-indigo-50/75 border border-indigo-100 p-2.5 rounded-lg flex justify-between items-center text-xs">
                    <div>
                      <span className="text-[9px] text-indigo-400 block font-bold uppercase tracking-wider">KODE UNDANGAN</span>
                      <strong className="text-indigo-800 font-mono tracking-widest text-sm">{currentGroup.kodeUndangan || 'BELUM_ADA'}</strong>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        navigator.clipboard.writeText(currentGroup.kodeUndangan || '');
                        alert(`Kode undangan "${currentGroup.kodeUndangan}" berhasil disalin!`);
                      }}
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-3 text-xs">
                    <div>
                      <span className="text-[10px] text-slate-400 block">Plafon Maks</span>
                      <strong className="text-[#1F3864]">{formatIdr(currentGroup.plafonMaks)}</strong>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-400 block">Kehadiran Pertemuan</span>
                      <strong className="text-[#2F6B6B]">{currentGroup.kehadiranRate}%</strong>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#1F3864]/5 border border-[#1F3864]/20 p-3.5 rounded-xl space-y-2 text-xs text-slate-800">
                  <h5 className="font-extrabold text-[#1F3864] flex items-center gap-1">
                    <CheckCircle2 className="w-4 h-4 text-[#548235]" /> Pakta Saling Menjamin
                  </h5>
                  <p className="text-[11px] leading-relaxed text-slate-600">
                    "Kami berikrar untuk saling memotivasi usaha, saling membantu cicilan, dan bersedia bertanggung renteng (patungan menalangi) jika rekan kami tertimpa musibah/tunggakan."
                  </p>
                </div>

                {/* Group Members List */}
                <div className="space-y-2">
                  <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider">Daftar Anggota Kelompok</span>
                  {currentGroup.anggotaIds.map(mId => {
                    const m = members.find(x => x.id === mId);
                    if (!m) return null;
                    const mLoan = loans.find(l => l.memberId === m.id && l.status !== 'Lunas');
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <h6 className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                              {m.nama} 
                              {m.id === currentGroup.ketuaId && <span className="text-[8px] bg-[#1F3864] text-white px-1.5 rounded font-black uppercase">Ketua</span>}
                            </h6>
                            <span className="text-[10px] text-slate-400">{m.pekerjaan} · Skor {m.skorKeanggotaan}</span>
                          </div>
                        </div>
                        <span className="text-[9px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">{statusText}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#1F3864]/5 p-4 rounded-xl border border-[#1F3864]/20 space-y-2.5">
                  <h4 className="text-xs font-bold text-[#1F3864] uppercase flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-amber-500" /> Gabung via Kode Undangan
                  </h4>
                  <p className="text-[10px] text-slate-600">
                    Punya kode dari ketua kelompok? Masukkan di bawah untuk bergabung secara langsung.
                  </p>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-3">
                  <h4 className="text-xs font-bold text-slate-700 uppercase">Buat Kelompok Baru</h4>
                  <form onSubmit={handleCreateGroup} className="space-y-3 text-xs">
                    <div>
                      <label className="block text-slate-600 mb-1">Nama Kelompok</label>
                      <input
                        type="text"
                        placeholder="Contoh: Flamboyan, Melati"
                        value={newGroupForm.nama}
                        onChange={(e) => setNewGroupForm({ ...newGroupForm, nama: e.target.value })}
                        className="w-full border border-slate-200 rounded px-2 py-1.5"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-slate-600 mb-1">Plafon Maksimal (Rp)</label>
                        <select
                          value={newGroupForm.plafon}
                          onChange={(e) => setNewGroupForm({ ...newGroupForm, plafon: e.target.value })}
                      ...
                      <div>
                        <label className="block text-slate-600 mb-1">Pertemuan Rutin</label>
                        <input
                          type="text"
                          value={newGroupForm.jadwal}
                          onChange={(e) => setNewGroupForm({ ...newGroupForm, jadwal: e.target.value })}
                          className="w-full border border-slate-200 rounded px-2 py-1.5"
                        />
                      </div>
                    </div>
                    <button type="submit" className="w-full bg-[#2F6B6B] text-white font-bold py-2 rounded">
                ...
                <div className="space-y-2">
                  <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider">Gabung Grup Terbuka Sekitar Anda</span>
                  {groups.map(g => (
                    <div key={g.id} className="bg-white p-3 rounded-xl border border-slate-200 flex justify-between items-center text-xs">
                      <div>
                        <h5 className="font-bold text-slate-800">{g.nama}</h5>
                        <p className="text-[10px] text-slate-400 mt-0.5">Pertemuan bulanan: {g.jadwalPertemuan}</p>
                      </div>
                      <button
        ...
        {activeTab === 'pinjaman' && (
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-base font-bold text-slate-800 mt-2">Pengajuan Pembiayaan</h3>
              <p className="text-xs text-slate-500">Transparan, Terkait Kelompok Tanggung Renteng</p>
            </div>
            ...
            {currentLoan ? (
              <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-4">
                <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                  <div>
                    <span className="text-[9px] font-bold text-slate-400 block">STATUS PENGAJUAN</span>
                    <h4 className="text-sm font-bold text-[#1F3864] mt-0.5">{currentLoan.status}</h4>
                  </div>
                  <div className="text-right">
                ...
                <div className="text-xs space-y-2">
                  <div className="flex justify-between"><span className="text-slate-400">Plafond</span><strong className="text-slate-800">{formatIdr(currentLoan.nominal)}</strong></div>
                  <div className="flex justify-between"><span className="text-slate-400">Tenor</span><span className="text-slate-800">{currentLoan.tenor} Bulan</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Tujuan</span><span className="text-slate-800">{currentLoan.tujuan}</span></div>
                </div>

                {/* AI Early Warning Alert Frame */}
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-1.5 text-xs">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">Hasil Skrining AI EWS</span>
                  <div className="flex items-center gap-1.5">
                    <span className={`w-3 h-3 rounded-full ${
                      currentLoan.flagAi === 'HIJAU' ? 'bg-[#548235]' : currentLoan.flagAi === 'KUNING' ? 'bg-[#C55A11]' : 'bg-[#C0392B]'
                    }`}></span>
                    <strong className="text-slate-800">Rekomendasi: {currentLoan.flagAi}</strong>
                  </div>
                ...
                {currentLoan.status === 'Diajukan' && (
                  <div className="border-t border-slate-100 pt-3 space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-bold text-[#C55A11] uppercase">Ada kesalahan flag AI?</span>
                      <button
                    ...
                    {showSanggahArea && (
                      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 space-y-2 text-xs">
                        <label className="block font-bold text-slate-700">Tuliskan Sanggahan Anda</label>
                        <textarea
                          rows={3}
                          value={sanggahText}
                    ...
                    {currentLoan.isSanggah && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-[10px] text-blue-900">
                        <strong>Sanggahan Terdaftar:</strong> "{currentLoan.sanggahAlasan}" — Status pengajuan ditangguhkan untuk diverifikasi manual oleh pengurus.
                      </div>
                    )}
                  </div>
                )}
              ...
              <form onSubmit={handleApplyLoan} className="bg-white p-4 rounded-xl border border-slate-200 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Nominal Pembiayaan (Rp)</label>
                  <input
                    type="number"
                    step="500000"
                    value={loanAmount}
                  ...
                  />
                  {currentGroup && (
                    <span className="text-[10px] text-slate-400 mt-1 block">Batas Plafon Kelompok Anda: <strong>{formatIdr(currentGroup.plafonMaks)}</strong></span>
                  )}
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Tujuan Penggunaan Dana</label>
                  <input
                    type="text"
                    value={loanPurpose}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Tenor Pengembalian</label>
                  <select
                    value={loanTenor}
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-[11px] text-[#C55A11] space-y-1">
              <h5 className="font-bold flex items-center gap-1"><Sparkles className="w-3.5 h-3.5 shrink-0" /> Transparansi Skrining Risiko</h5>
              <p className="text-slate-600 leading-relaxed">
                RantaiRenteng menyaring risiko dengan parameter transparan. Kami tidak mengorek riwayat ponsel pribadi Anda, melainkan melihat riwayat kelompok dan ketaatan tabungan Anda.
              </p>
            </div>
          </motion.div>
        )}
          ...
          >
            <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-4">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wide flex items-center gap-1">
                <User className="w-4 h-4 text-[#1F3864]" /> Akun & Registrasi e-KYC
              </h4>
              ...
              {kycStep === 1 ? (
                <div className="space-y-3">
                  <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 text-xs">
                    ...
                    <strong className="text-slate-800 text-sm flex items-center gap-1.5 mt-0.5">
                      {currentMember.statusKyc === 'Approved' ? (
                        <>
                          <CheckCircle2 className="w-4 h-4 text-[#548235]" /> Aktif (e-KYC Terverifikasi)
                        </>
                      ) : currentMember.statusKyc === 'Requested' ? (
                        <>
                          <AlertCircle className="w-4 h-4 text-[#C55A11]" /> Requested (Menunggu Persetujuan)
                        </>
                      ) : (
                        <>
                          <AlertCircle className="w-4 h-4 text-[#C0392B]" /> Belum Verifikasi
                        </>
                      )}
                    </strong>
                  </div>
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      ...
                      <span className="font-bold text-slate-800">{currentMember.nama}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      <span className="text-slate-400">NIK (Nomor Induk)</span>
                      <span className="font-bold text-slate-800">{currentMember.nik}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      <span className="text-slate-400">Pekerjaan</span>
                      <span className="font-bold text-slate-800">{currentMember.pekerjaan}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      <span className="text-slate-400">Alamat</span>
                      <span className="font-bold text-slate-800">{currentMember.alamat}</span>
                    </div>
                  </div>
                ...
                <form onSubmit={handleSubmitKyc} className="space-y-3.5 text-xs">
                  <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                    <span className="font-bold text-slate-700">Form e-KYC Digital (Step 2)</span>
                    <button type="button" onClick={() => setKycStep(1)} className="text-[#C0392B] font-bold">Kembali</button>
                  </div>

                  <div>
                    <label className="block text-slate-600 mb-1">Nama Lengkap Sesuai KTP</label>
                    <input
                      type="text"
                      required
                      value={kycForm.nama}
                      onChange={(e) => setKycForm({ ...kycForm, nama: e.target.value })}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-slate-600 mb-1">Nomor Induk Kependudukan (NIK)</label>
                      <input
                        type="text"
                        required
                        maxLength={16}
                    ...
                    <div>
                      <label className="block text-slate-600 mb-1">No. Handphone</label>
                      <div className="flex gap-1">
                        <input
                          type="text"
                          required
                          value={kycForm.noHp}
                          onChange={(e) => setKycForm({ ...kycForm, noHp: e.target.value })}
                  ...
                  {otpSent && !otpVerified && (
                    <div className="p-2 bg-amber-50 border border-amber-200 rounded space-y-1.5">
                      <label className="block text-slate-600 font-bold">Masukkan Kode OTP</label>
                      <div className="flex gap-2">
                        <input
                          type="text"
                          value={kycForm.otp}
                          onChange={(e) => setKycForm({ ...kycForm, otp: e.target.value })}
                  ...
                  <div>
                    <label className="block text-slate-600 mb-1">Alamat Domisili / Desa</label>
                    <input
                      type="text"
                      required
                      value={kycForm.alamat}
                      onChange={(e) => setKycForm({ ...kycForm, alamat: e.target.value })}
                      className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs bg-slate-50"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-slate-600 mb-1">Pekerjaan Utama</label>
                      <input
                        type="text"
                        required
                        value={kycForm.pekerjaan}
                        onChange={(e) => setKycForm({ ...kycForm, pekerjaan: e.target.value })}
                        className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs bg-slate-50"
                      />
                    </div>
                    <div>
                      <label className="block text-slate-600 mb-1">Peran Koperasi</label>
                      <select
                        value={kycForm.peran}
                        onChange={(e: any) => setKycForm({ ...kycForm, peran: e.target.value })}
                        className="w-full border border-slate-200 rounded p-1.5 bg-white"
                      >
                  ...
                  <div className="border-2 border-dashed border-slate-300 rounded-lg p-3 text-center bg-slate-50 relative">
                    ...
                    <span className="text-[10px] text-slate-500 font-bold block mt-1">
                      {kycForm.ktpFile ? kycForm.ktpFile : 'Unggah Foto KTP & Swafoto'}
                    </span>
                    <input
                      type="file"
                      accept="image/*"
                  ...
                  <button
                    ...
                    className={`w-full text-white font-extrabold py-2.5 rounded-xl text-xs transition-colors ${
                      otpSent && !otpVerified ? 'bg-slate-300 cursor-not-allowed' : 'bg-[#F06A6A] hover:bg-[#E5544F]'
                    }`}
                  >
                    Kirim Form e-KYC
                  </button>
      ...
      <div className="bg-[#1E1F21] text-slate-400 px-5 pt-3 pb-2 flex flex-col items-center gap-2.5 shrink-0 z-20 select-none border-t border-white/5">
        <div className="flex items-center justify-between w-full text-[8px] font-bold tracking-widest text-slate-400">
          <div className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-full bg-[#F06A6A] border border-white/40 flex items-center justify-center text-[5px] text-white font-serif">印</span>
        ...
        <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-xs p-5 text-center shadow-2xl border border-slate-200/60 relative overflow-hidden">
            ...
            <div className="bg-[#F06A6A] text-white p-3 -mx-5 -mt-5 mb-4 flex flex-col items-center gap-0.5 select-none">
              <span className="text-[8px] tracking-widest font-black uppercase text-amber-200">GERBANG QRIS DIGITAL</span>
              <h4 className="text-xs font-black tracking-wide uppercase">KOPERASI MERAH PUTIH</h4>
            </div>
            
            <p className="text-[11px] text-[#6D6E6F] font-bold uppercase tracking-wider">{qrisConfig.title}</p>
            <strong className="text-xl font-black text-[#1E1F21] block mt-1">{formatIdr(qrisConfig.amount)}</strong>
            ...
            <div className="w-32 h-32 border-4 border-[#1E1F21] mx-auto my-4 bg-white p-2.5 flex flex-col justify-between items-center relative rounded-xl shadow-inner">
              <div className="grid grid-cols-4 gap-1 w-full h-full opacity-90">
                {Array.from({ length: 16 }).map((_, i) => (
                  <div key={i} className={`rounded-sm ${i % 3 === 0 || i % 5 === 0 ? 'bg-[#F06A6A]' : 'bg-white'}`}></div>
                ))}
              </div>
            ...
            <div className="text-[10px] text-slate-400 font-medium px-2 leading-relaxed">
              Scan barcode otomatis terhubung ke immutable ledger Koperasi Desa. Bebas biaya admin (gas Relayer).
            </div>
